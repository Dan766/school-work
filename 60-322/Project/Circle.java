...
public class Circle extends Shape{
	...
	public Circle(Color currentColor){
		super(currentColor, new Coordinates(200, 200));
		diameter = 50;
	}
	...
	public boolean onThePerimeter(Coordinates mousePosition){	
		...
		centreOfCircle = new Coordinates(this.getReferencePoint().getX() + diameter/2,
				                         this.getReferencePoint().getY() + diameter/2);
	...
	public boolean shapeIsSelected(Coordinates positionOfMouse){ 
		...
		Coordinates centreOfCircle;
		centreOfCircle = computeCenterOfCircle(this.getReferencePoint(), diameter);
	...
	public void changeShape(int currentPhase, int size){
		...
		if (currentPhase == 3){
			newReferencePointX = this.getReferencePoint().getX() + (diameter - size)/2;
			newReferencePointY = this.getReferencePoint().getY() + (diameter - size)/2;
			this.setReferencePoint(new Coordinates(newReferencePointX, newReferencePointY));
			diameter = size; 
		} 
	...
	public void showMe(Graphics g){
		...
		g.drawOval(this.getReferencePoint().getX(), // Draw a circle with the specified 
				// corner point and diameter
				this.getReferencePoint().getY(),   
				diameter,
				diameter);
	}
	...
	public String toString(){
		return ("Circle with reference point " + this.getReferencePoint() + " having diameter " + diameter + "\n");
	}
